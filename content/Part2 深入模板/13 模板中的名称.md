...
(X<1)>0
```
* 先让X和1比大小，然后把结果转换成1或0，再与0比较大小。因此解析器先查找<前的名称，是模板名称时才会\<看作左尖括号，其他情况则看作小于号
```cpp
template<bool B>
class X {
public:
...
## [依赖型类型的名称](https://en.cppreference.com/w/cpp/language/dependent_name)与typename前缀
...
## [依赖型模板的名称](https://en.cppreference.com/w/cpp/language/dependent_name)与.template
...
public:
    void case1(A<T>::template B<N>::template C<N>* p) {
        p->template C<N>::f(); // 禁止虚调用
    }
    void case2(A<T>::template B<N>::template C<N>& p) {
        p.template C<N>::f(); // 禁止虚调用
    }
};
```
...
## [依赖型名称](https://en.cppreference.com/w/cpp/language/dependent_name)与using声明
...
```
* 编译器必须先知道`<3>`是模板实参列表才能知道p是函数实参，先知道f是模板才能知道`<3>`是模板实参列表，这就陷入了死循环。解决这一问题的方法是加入
```cpp
template<typename T> void f();
```
虽然这一定义不会对调用`f(X*)`有帮助，但可以帮助编译器识别`f<3>`是模板id，然后ADL就会发现N::f然后成功调用。
